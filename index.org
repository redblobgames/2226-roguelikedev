...
In Javascript, ~{ … }~ works both as a map (Python dict) and an object. When a key ='ArrowRight'=  comes in, I can call ~actions[event.key]()~ to run the corresponding action. When I switch to wall-building mode, I'll assign a different key map to =actions=.
