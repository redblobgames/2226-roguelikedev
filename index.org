...
So maybe I shouldn't have the player move around while building. What if a build key ~B~ starts build mode, and then you can move the cursor around freely? Then pressing ~B~ again goes back to the player. This would be /placing orders/ that your ally NPCs would actually build. I might try this, but I'll need NPCs to make it work.
