...
The generated map is only the wilderness. In this game the player will be building the rooms. I need to figure out the UI for that. If I had a /facing/ direction then the player could build in front of themselves. But the keystrokes for that are annoying. If you want to build a wall on tiles 1, 2, 3:
...
You have to move to 4, press ~Up~ to face towards 1, press ~E~ to build, then press ~Right~ to face right, then press ~Right~ to move right, then press ~Up~ to face towards towards 2, press ~E~ to build, etc.

I also considered having the player /pick up/ building materials and then place them for building, but I think that's even more awkward.

So maybe I shouldn't have the player move around while building. What if a build key ~B~ starts build mode (like vim's insert vs normal mode), and then you can move the cursor around freely to build things? Then pressing ~B~ again goes back to the player. This would be /placing orders/ that your ally NPCs would actually build. For now I can have it build immediately without waiting for NPCs, and then change it once I add NPCs.
...
<x:footer>
  Created 28 Jun 2022; &#160;
  <!-- hhmts start -->Last modified: 08 Jul 2022<!-- hhmts end -->
</x:footer>
#+end_export
